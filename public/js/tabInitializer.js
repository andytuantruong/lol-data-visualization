...
function restoreActiveTab() {
  // Get stored tab preference from localStorage if available, otherwise default to player-chart
  const activeTabId = localStorage.getItem('activeTab') || 'player-chart';

  // Activate the tab
  const tab = document.querySelector(`.tab[data-tab="${activeTabId}"]`);
  if (tab) {
    tab.click();
  }
}
